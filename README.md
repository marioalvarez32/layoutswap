...
**Status:** pre-alpha. The app opens, reads the connected monitors, captures the
arrangement Windows shows as a named layout with its summary and schematic, and writes
a self-contained switch script per layout. Running that switch from inside the app is
the next slice. The reference scripts under `samples/` do the job today by hand.
