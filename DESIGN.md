...
| 2026-09-10 | Q3, console half: a shortcut launch prints the needs-you band's two lines once ("Press the input button on <alias>, or turn the console off." and "Waiting until <alias> is Available") and then a countdown line every ten seconds; the app reads the same lines through a fifth progress status, needs-you, emitted every second with the seconds left | 1i, 2c |
| 2026-09-10 | Steps reorder with Move up and Move down on the expanded row, keyboard-friendly, no drag in the steps slice; the timeline is one list with Check monitors, Apply arrangement and Verify as fixed rows and Add step before and Add step after around the apply | 2e |
...
| 2026-09-10 | An expanded step row starts with a Step select (wait a number of seconds, send an input source to a monitor) that converts the row in place; the send row reads "Send <input> to <monitor> then <wait rule>" as inline selects with the rule's timeout beside it ("up to 5 s"), "(now)" marks the monitor's current input, and "Other code" opens a hex field; the drop wait sits in a collapsed Advanced section under the timeline whose summary reads "Advanced: drop wait 5 s" | 2e, code |
